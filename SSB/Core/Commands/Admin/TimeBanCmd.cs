    ...
    public class TimeBanCmd : IBotCommand
    {
        ...
        private async Task AddBan(CmdArgs c)
        {
            ...
            if (_banDb.UserAlreadyBanned(c.Args[2]))
            {
                ...
                await _ssb.QlCommands.QlCmdTell(
                    string.Format(
            ...
            else
            {
                await
                    _ssb.QlCommands.QlCmdTell(
                        string.Format(
                            "^1[ERROR]^3 An error occurred while attempting to add a time-ban for player: {0}",
                            c.Args[2]), c.FromUser);
            }
        }
        ...
        private async Task DelBan(CmdArgs c)
        {
            try
            {
                ...
                await
                    _ssb.QlCommands.QlCmdSay(string.Format("^2[SUCCESS]^7 Removed time-ban for player {0}",
                        c.Args[2]));
            ...
            await
                _ssb.QlCommands.QlCmdTell(
                    string.Format(
                        "^1[ERROR]^3 An error occurred while attempting to remove time ban for player: {0}",
                        c.Args[2]), c.FromUser);
        }
        ...
        private async Task EvalBanAddition(CmdArgs c)
        {
            ...
            {
                await _ssb.QlCommands.QlCmdTell("^1[ERROR]^3 Time must be a positive number!", c.FromUser);
                return;
            }
            if (time <= 0)
            {
            ...
            if (time > int.MaxValue)
            {
                ...
                await _ssb.QlCommands.QlCmdTell("^1[ERROR]^3 Time is too large!", c.FromUser);
                return;
            }
            ...
            if (_userDb.GetUserLevel(c.Args[2]) >= UserLevel.Admin)
            {
                await
                    _ssb.QlCommands.QlCmdTell(
                        "^1[ERROR]^3 Users with user level admin or higher may not be time-banned!",
                        c.FromUser);
                return;
            }
            await AddBan(c);
        ...
        private async Task EvalBanDeletion(CmdArgs c)
        {
            ...
            {
                await
                    _ssb.QlCommands.QlCmdTell(string.Format("^1[ERROR]^3 No timeban exists for user: {0}",
                        c.Args[2]), c.FromUser);
            }
            else
            {
            ...
            await _ssb.QlCommands.QlCmdSay(string.Format("^5[TIMEBAN]^7 {0}",
                ((!string.IsNullOrEmpty(bans))
                    ? (string.Format("Banned players: ^1{0}^7 - To see ban info: ^3{1}{2} check <player>",
                        bans, CommandProcessor.BotCommandPrefix, c.CmdName))
