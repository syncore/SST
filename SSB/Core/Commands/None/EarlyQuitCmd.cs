...
namespace SSB.Core.Commands.None
{
    ...
    public class EarlyQuitCmd : IBotCommand
    {
        ...
        {
            long totalQuits = _quitDb.GetUserQuitCount(c.Args[2]);
            string qStr = totalQuits > 0 ? string.Format("^1{0}^7 early quits", totalQuits) : "^2no^7 early quits";
            long quitsRemaining = (_ssb.Mod.EarlyQuit.MaxQuitsAllowed - totalQuits);
            await
                _ssb.QlCommands.QlCmdSay(
                    string.Format("^5[EARLYQUIT] ^3{0}^7 has {1}. ^1{2}^7 remaining before banned for ^1{3} {4}",
        ...
        private async Task ListQuits(CmdArgs c)
        {
            ...
            await _ssb.QlCommands.QlCmdSay(string.Format("^5[EARLYQUIT]^7 {0}",
                ((!string.IsNullOrEmpty(quits)) ? (string.Format("Early quitters: ^1{0}^7 - To see quits remaining: ^3{1}{2} check <player>",
                quits, CommandProcessor.BotCommandPrefix, c.CmdName)) : "No players have quit early.")));
        }
    }
}