...
namespace SST.Core.Modules
{
    ...
    public class PickupPlayers
    {
        ...
        public PickupPlayers(SynServerTool sst, PickupManager manager)
        {
            _sst = sst;
            _manager = manager;
        }
        ...
        public async Task<bool> ProcessAddPlayer(string player)
        {
            ...
            if (_manager.IsPickupPreGame && !_manager.AreTeamsFull)
            {
                ...
                if ((_manager.AvailablePlayers.Count >= _sst.Mod.Pickup.Teamsize * 2) &&
            ...
            await
                _sst.QlCommands.QlCmdTell("^1[ERROR]^3 A pickup has not been set to start.",
                    player);
