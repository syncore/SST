...
namespace SST.Core.Commands.SuperUser
{
    ...
    public class AllReadyCmd : IBotCommand
    {
        ...
        private readonly string _logPrefix = "[CMD:ALLREADY]";
        ...
        public AllReadyCmd(SynServerTool sst)
        {
            _sst = sst;
        }
        ...
        {
            await _sst.QlCommands.SendToQlAsync("allready", false);
            StatusMessage = "^2[SUCCESS]^7 Attempted to send allready command.";
            await SendServerTell(c, StatusMessage);
            Log.Write("Attempted to force all players to ready.", _logClassType, _logPrefix);
            return true;
        }
